...
//		request.headers().get
		...
		final String path = santizerUri(uri);
		if (path == null) {
		...
		sendFileFuture = ctx.write(new ChunkedFile(randomAccessFile, 0, fileLength, 8192*1024), ctx.newProgressivePromise());

		sendFileFuture.addListener(new ChannelProgressiveFutureListener() {

			@Override
			public void operationComplete(ChannelProgressiveFuture future) throws Exception {
				System.out.println("Transfer cmplete.");
			}
		...
		if (!HttpUtil.isKeepAlive(request)) {
			lastContentFuture.addListener(ChannelFutureListener.CLOSE).sync();
		}
	}

	private String santizerUri(String uri) {
		try {
